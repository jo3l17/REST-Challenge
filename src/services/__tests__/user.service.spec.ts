...
describe('User service', () => {
  ...
  describe('findVerifiedById', () => {
    it('should return an user', async () => {
      const userFound = await UserService.findVerifiedById(
  ...
  describe('updateVerification', () => {
    ...
    it('should return an user moderator', async () => {
      const userUpdated = await UserService.updateVerification(
        user.id,
        'moderator',
      );
  ...
  describe('createTemporalEmail', () => {
    it('should update the temporal email', async () => {
      expect.assertions(2);
      const updatedUser = await UserService.createTemporalEmail(
        authenticatedUserwithNoTemp.id,
        'newemail@gmail.com',
      );
    ...
    it('should throw email already in use', async () => {
      expect.assertions(2);
      try {
        await UserService.createTemporalEmail(
          authenticatedUserwithNoTemp.id,
          user.email,
        );
      } catch (e) {
